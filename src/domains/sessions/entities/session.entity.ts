...
export class SessionEntity extends BaseEntity {
  ...
  branchId?: string;     // optional if branch-level differentiation needed
  userId!: string;
  ...
  constructor(partial?: Partial<SessionEntity>) {
    ...
    if (partial) {
      ...
      if (partial.state) this.state = new SessionStateVO(partial.state.value);
      if (partial.items) this.items = partial.items.map(i => new CartItemEntity(i as any));
    }
  }
  ...
  removeItem(productId: string) {
    ...
    this.state.set(this.items.length ? SessionState.CART_UPDATED : SessionState.BROWSING_MENU);
    this.touch();
  }
  ...
  get totalAmount(): number {
    const subtotal = this.items.reduce((sum, i) => sum + i.total, 0);
    return this.discount ? this.discount.apply(subtotal) : subtotal;
  }
