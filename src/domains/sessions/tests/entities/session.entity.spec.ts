...
describe('SessionEntity', () => {
  ...
  it('should increase quantity if same product added again', () => {
    const item = new CartItemEntity({ productId: 'p1', name: 'Pizza', quantity: 2, price: 10 });
    session.addItem(item);
    session.addItem(new CartItemEntity({ productId: 'p1', name: 'Pizza', quantity: 3, price: 10 }));
  ...
  it('should calculate total with discount', () => {
    ...
    session.addItem(new CartItemEntity({ productId: 'p2', name: 'Soda', quantity: 1, price: 5 }));

    const discount = new DiscountVO({ type: 'PERCENTAGE', value: 10 });
    session.applyDiscount(discount);

    expect(session.totalAmount).toBeCloseTo(22.5); // (2*10 + 1*5) * 0.9
  });

  it('should checkout', () => {
    session.addItem(new CartItemEntity({ productId: 'p1', name: 'Pizza', quantity: 1, price: 10 }));
    session.checkout();
