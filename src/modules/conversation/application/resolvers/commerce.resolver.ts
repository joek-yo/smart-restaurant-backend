...
export class CommerceResolver implements IntentResolver, OnModuleInit {
  ...
  private readonly patterns: { pattern: RegExp; intent: ConversationIntent }[] = [
    { pattern: /\b(add|want|order|buy|get)\b.*(item|product|food|dish)/i, intent: ConversationIntent.ADD_TO_CART },
    ...
    { pattern: /\b(show|view|see|browse|list)\b.*(menu|products|items|food)/i, intent: ConversationIntent.VIEW_PRODUCTS },
